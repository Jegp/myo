...
class MyoRaw(object):
    ...
    def detect_tty(self):
        for p in comports():
            if re.search(r'PID=2458:0*1', p[2]):
                logging.debug('using device:', p[0])
                return p[0]
    ...
    def connect(self):
        ...
        logging.debug('firmware version: %d.%d.%d.%d' % (v0, v1, v2, v3))
        ...
        if self.old:
            ...
            logging.debug('device name: %s' % name.payload)
        ...
        def handle_data(p):
            ...
            if attr == 0x27:
                ...
                logging.debug('data with unknown attr: %02X %s' % (attr, p))
...
if __name__ == '__main__':
    ...
    def proc_emg(emg, moving, times=[]):
        ...
        ## logging.debug(framerate of received data)
        times.append(time.time())
        if len(times) > 20:
            #logging.debug((len(times) - 1) / (times[-1] - times[0]))
            times.pop(0)

    m.add_emg_handler(proc_emg)
    m.connect()

    m.add_arm_handler(lambda arm, xdir: logging.debug('arm', arm, 'xdir', xdir))
    m.add_pose_handler(lambda p: logging.debug('pose', p))
